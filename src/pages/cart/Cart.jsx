...
function Cart() {
  ...
  return (
    <Layout>
      <div
        className={`h-screen bg-gray-100 pt-5 ${mode === 'dark' ? 'bg-dark-mode' : ''}`}
        ...
        <div className="mx-auto max-w-5xl px-6 md:flex md:space-x-6 xl:px-0">
          <div className="rounded-lg md:w-2/3">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className={`justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start ${mode === 'dark' ? 'bg-dark-mode' : ''}`}
                style={{ color: mode === 'dark' ? 'white' : '' }}
              >
                <img
